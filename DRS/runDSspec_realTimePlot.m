...
%     s1.Terminator = "CR/LF";
    ...
    amp1=[sin(2*pi* freq*values) zeros(1,length(values)*2) sin(2*pi* freq*values)];
    amp2=sin(2*pi*freq*values);
...
%             fscanf(s1);
        ...
        if doPlot && n>1
            ...
            if n>2
                subplot(2, 2, 4);
                CWdata.II=[...
                    mean(S1D1(n-2, i1:i2, :), 3).',...
                    mean(S1D2(n-2, i1:i2, :), 3).',...
                    mean(S2D2(n-2, i1:i2, :), 3).',...
                    mean(S2D1(n-2, i1:i2, :), 3).'];
                muaPrev=analyzeCWdata(...
                    CWdata, musp0*(CWdata.lambda/lam0).^-b);
                plot(CWdata.lambda, mua-muaPrev, 'k-');
                xlim([lamRng(1), lamRng(2)]);
                xlabel('\lambda (nm)');
                ylabel('\mu_{a,n}-\mu_{a,n-1} (1/mm)');
                drawnow;
            end
